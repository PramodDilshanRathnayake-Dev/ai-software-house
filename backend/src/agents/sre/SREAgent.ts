...
export class SREAgent {
     ...
     */
    async monitorAndHeal(missionId: string, stackTrace: string): Promise<void> {
        ...
        await MissionContext.updateOne(
            { projectId: missionId, "incidents.id": incidentId },
            { $set: { "incidents.$.status": "FIXING" } }
        );
        ...
        sandboxRunnerInstance.run(missionId).catch((err: any) => {
    ...
    private emitStatusUpdate(missionId: string, status: string, message: string) {
        const io = getSocket();
        if (io) {
